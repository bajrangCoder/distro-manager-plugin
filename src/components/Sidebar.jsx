...
export default function Sidebar({ plugin }) {
	...
	const checkActiveServers = async () => {
		...
		const distroIds = Object.keys(
			plugin.manager.getAvailableDistros().reduce((acc, d) => {
				acc[d.id] = d;
				return acc;
			}, {}),
		);
	...
	const filteredDistros = distros.filter(
		(d) =>
			...
			<div className="sidebar-header">
				<div className="sidebar-title-container">
					...
					<span className="arch-tag">
						Arch: {plugin.manager.arch || "loading..."}
					</span>
				</div>
				<button
			...
			<div className="distro-list">
				{filteredDistros.length > 0 ? (
					filteredDistros.map((d) => {
						...
						if (isBusy) {
							statusText =
								state.status === "installing" ? "Installing..." : "Removing...";
							statusClass = "busy";
						} else if (isRunning) {
						...
						return (
							<div
								className={`distro-card ${isRunning ? "active-distro" : ""}`}
								key={d.id}
							>
								{/* Accordion Header */}
								<div
									className="distro-card-header"
								...
								{isExpanded && (
									<div className="distro-card-body">
											...
											{isInstalled && (
												<div className="detail-row">
													...
													<span
														...
														title={`${plugin.manager.distrosPath}/${d.id}`}
													>
														{d.id}
													</span>
												</div>
										...
										{isBusy && state.logs && (
											<Fragment>
												...
												<div
													...
													ref={(el) => (logRefs.current[d.id] = el)}
												>
										...
										{state && state.status === "error" && (
											<Fragment>
												<div className="console-title" style="color: #f44336;">
													Error Output
												</div>
												<div
													className="console-logs"
													...
													{hasAcodeX ? (
														<div className="btn-group">
															<button
																...
																onClick={() =>
																	triggerLaunch(d.id, defaultTerminal)
																}
